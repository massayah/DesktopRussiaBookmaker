...
<head>
<title>Russia BookMaker - Administration</title>
<meta charset="utf-8">
...
{
	$access = $bdd->prepare('SELECT isadmin FROM russia_user WHERE username = ?');
...
$result = $bdd->query('SELECT * FROM russia_schedule es ORDER BY ISNULL(es.group), es.group, es.id');
...
FROM russia_schedule es LEFT OUTER JOIN russia_team et1 ON et1.id = es.team1 LEFT OUTER JOIN russia_team et2 ON et2.id = es.team2 
...
<?php
  $huitieme = $bdd->query('SELECT tempname1, tempname2, id FROM russia_schedule WHERE id BETWEEN 37 AND 44');
  ...
  ?>
      ...
      <?php
	$winnershuitieme = $bdd->query('SELECT name FROM russia_team');
	while ($winnershuitiemedata = $winnershuitieme->fetch())
	{
      ...
      <option value="first"></option>
      <?php
	$winnershuitieme2 = $bdd->query('SELECT name FROM russia_team');
...
<?php
  $quart = $bdd->query('SELECT tempname1, tempname2, id FROM russia_schedule WHERE id BETWEEN 45 AND 48');
  ...
  ?>
      ...
      <option value="first"></option>
      <?php
	$winnersquart = $bdd->query('SELECT name FROM russia_team');
	while ($winnersquartdata = $winnersquart->fetch())
	{
      ...
      <option value="first"></option>
      <?php
	$winnersquart2 = $bdd->query('SELECT name FROM russia_team');
...
<?php
  $demi = $bdd->query('SELECT tempname1, tempname2, id FROM russia_schedule WHERE id BETWEEN 49 AND 50');
  ...
  ?>
      ...
      <option value="first"></option>
      <?php
	$winnersdemi = $bdd->query('SELECT name FROM russia_team');
	while ($winnersdemidata = $winnersdemi->fetch())
	{
      ...
      <option value="first"></option>
      <?php
	$winnersdemi2 = $bdd->query('SELECT name FROM russia_team');
...
<?php
  $finale = $bdd->query('SELECT tempname1, tempname2, id FROM russia_schedule WHERE id = 51');
  ...
  ?>
      ...
      <option value="first"></option>
      <?php
	$winnersfinale = $bdd->query('SELECT name FROM russia_team');
	while ($winnersfinaledata = $winnersfinale->fetch())
	{
      ...
      <option value="first"></option>
      <?php
	$winnersfinale2 = $bdd->query('SELECT name FROM russia_team');
...
<tbody>
<?php
$users = $bdd->query('SELECT russia_user.username, points, team1, team2, team3, team4 FROM russia_user LEFT OUTER JOIN russia_top ON russia_user.username = russia_top.username WHERE russia_user.isadmin = 0');
$i = 1;
