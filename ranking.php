...
<?php
$usergroup = $bdd->prepare('SELECT team FROM russia_user WHERE username = ?');
$usergroup->execute(array($_SESSION['username']));
$usergroupdata = $usergroup->fetch();
if ($usergroupdata['team'] == "0")
	$users = $bdd->query('SELECT russia_user.username, points, team1, team2, team3, team4 FROM rugby_user LEFT OUTER JOIN russia_top ON russia_user.username = russia_top.username WHERE russia_user.isadmin = 0 ORDER BY points DESC');
else
{
	$teamuser = $usergroupdata['team'];
	$users = $bdd->prepare('SELECT russia_user.username, points, team1, team2, team3, team4 FROM russia_user LEFT OUTER JOIN russia_top ON russia_user.username = russia_top.username WHERE russia_user.isadmin = 0 AND russia_user.team = ? ORDER BY points DESC');
	if ($teamuser == "2")
