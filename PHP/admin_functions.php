...
for ($i = 1; $i <= 51; $i++)
{
	if (isset($_POST['submit_match' . $i]) || isset($_POST['reset_match' . $i]))
	{
		...
		if (isset($_POST['reset_match' . $i]))
		{
			...
			$setovertime = $bdd->prepare('UPDATE russia_schedule SET overtime = ? WHERE id = ?');
		...
		if ($result1 != NULL && ( $result1 != "") && $result2 != NULL && ( $result2 != ""))
			{
				...
				$fillmatch = $bdd->prepare('UPDATE russia_schedule SET team1result = ?, team2result =  ? WHERE id = ?');
				...
				else
				{
					...
					$resultmatch = $bdd->prepare('SELECT team1, team2 FROM russia_schedule WHERE id = ?');
					$resultmatch->execute(array($i));
					$resultmatchdata = $resultmatch->fetch();
					
					$getusers = $bdd->prepare('SELECT russia_bet.username, points FROM russia_bet JOIN russia_user ON russia_user.username = russia_bet.username WHERE win = ? AND match_id = ?');
					$addpoints = $bdd->prepare('UPDATE russia_user SET points = ? WHERE username = ?');
					
					$ispoints = $bdd->prepare('SELECT russia_schedule.group FROM russia_schedule WHERE id = ?');
					$ispoints->execute(array($i));
					$ispointsdata = $ispoints->fetch();
					if ($ispointsdata['group'] != NULL)
					{
						$teamstate = $bdd->prepare('SELECT points FROM russia_team WHERE name = ?');
						...
						$teampoints = $bdd->prepare('UPDATE russia_team SET points = ? WHERE name = ?');
					}
					...
					else
					{
						...
						else
						{
							...
							{
								$filltab = $bdd->prepare('UPDATE russia_schedule SET team1penalty = ?, team2penalty = ? WHERE id = ?');
	...
	{
		$state = $bdd->prepare('SELECT available FROM russia_schedule WHERE id = ?');
		$blockbet = $bdd->prepare('UPDATE russia_schedule SET available = ? WHERE id = ?');
...
for ($i = 37; $i <= 44; $i++)
{
  for ($j = 1; $j <= 2; $j++)
  {
	if (isset($_POST['submit_huitieme' . $i .$j]))
	{
		...
		{
			$idteam = $bdd->prepare('SELECT id FROM russia_team WHERE name = ?');
			...
			if ($j == 1)
			{
				...
				$fillwinner = $bdd->prepare('UPDATE russia_schedule SET team1 = ? WHERE id = ?');
				$fillwinner->execute(array($idteamdata['id'], $i));
				$fillwinner->closeCursor();
			}
			else
			{
				$fillwinner = $bdd->prepare('UPDATE russia_schedule SET team2 = ? WHERE id= ?');
				$fillwinner->execute(array($idteamdata['id'], $i));
				$fillwinner->closeCursor();
			}
		}
...
if (isset($_POST['submit_finale511']))
{
	if (isset($_POST['update_finale_team511']) && !empty($_POST['update_finale_team511']))
	{
		...
		$idteamdata = $idteam->fetch();
		$fillwinner = $bdd->prepare('UPDATE russia_schedule SET team1 = ? WHERE id = 51');
...
if (isset($_POST['submit_finale512']))
{
	if (isset($_POST['update_finale_team512']) && !empty($_POST['update_finale_team512']))
	{
		...
		$idteamdata = $idteam->fetch();
		$fillwinner = $bdd->prepare('UPDATE russia_schedule SET team2 = ? WHERE id = 51');
		$fillwinner->execute(array($idteamdata['id']));
		$fillwinner->closeCursor();
	}
}
...
$userdelete = $bdd->query('SELECT username FROM russia_user');
while ($userdeletedata = $userdelete->fetch())
{
	if (isset($_POST['submit_delete_user_' . $userdeletedata['username']]))
	{
		$delete = $bdd->prepare('DELETE FROM russia_user WHERE username = ?');
